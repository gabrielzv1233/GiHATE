...
public sealed class MainForm : Form
{
    ...
    private void RefreshUi()
    {
        if (_config.Profile is null)
        {
            ...
            _detail.Text = "Run detection first. GiHATE will ask you to press the GiMATE button three times, then press a normal keyboard key as a safety check.";
            _apply.Enabled = false; _restore.Enabled = _config.Applied; return;
        }
    ...
    private void AdoptKnownProfile()
    {
        ...
        {
            MessageBox.Show(this, "The exact tested AORUS Master 16 device layout was not found unambiguously. Use guided detection instead.", "Known profile not available", MessageBoxButtons.OK, MessageBoxIcon.Information); return;
        }
    ...
    {
        _detection.Clear(); _pendingProfile = null; _mode = DetectionMode.Button; _detect.Enabled = false; _apply.Enabled = false;
        _status.Text = "Detection: press the GiMATE button 3 times";
        _detail.Text = "Do not press other keys during this step. GiHATE is looking for a repeated keyboard scan code plus a correlated vendor-defined HID report.";
    }
    ...
    private void OnKeyboard(KeyboardRawEvent e)
    {
        ...
        else if (_mode == DetectionMode.Safety && _pendingProfile is not null && _detection.NormalKeySafetyPassed(_pendingProfile))
        {
            var profile = _pendingProfile; _mode = DetectionMode.None; BeginInvoke(() => FinishDetection(profile));
        }
    }
    ...
    {
        if (profile.VendorUsagePage < 0xFF00 || string.IsNullOrWhiteSpace(profile.VendorInstanceId) || string.IsNullOrWhiteSpace(profile.KeyboardInstanceId))
        {
            MessageBox.Show(this, "The candidate failed GiHATE's vendor-HID safety checks.", "Detection failed", MessageBoxButtons.OK, MessageBoxIcon.Warning); _detect.Enabled = true; RefreshUi(); return;
        }
    ...
    private void ApplyConfiguration()
    {
        ...
        try
        {
            ...
            _config.TargetKey = targetName; _config.Applied = true; _config.Save();
    ...
    private void RestoreConfiguration()
    {
        ...
        try
        {
            ...
            _config.Applied = false; _config.Save();
